class Service {
  ...
  search = async(searchData) => {
   ...
   const findByModulesReadyCards = readyCards.filter(card => {
      const findModules = card.modules.filter(module.moduleTitle.toLowerCase() === searchData.toLowerCase());

      return !findModules.length ? false : true;
    });

   const findByCardsReadyCards = readyCards.filter(card => card.courseTitle.toLowerCase() === searchData.toLowerCase());

   return [...findByModulesReadyCards, ...findByCardsReadyCards];
  }
